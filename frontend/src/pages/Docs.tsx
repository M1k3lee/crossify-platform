  ...
  const sections = [
    ...
    },
    {
      ...
      content: [
        ...
        {
          title: 'TokenIDRegistry & GlobalSupplyTrackerV2 - Token ID System (January 2025)',
          description: 'We\'ve implemented a revolutionary Token ID system that enables true cross-chain price synchronization:\n\n🆔 TokenIDRegistry:\n• Maps token addresses to canonical bytes32 token IDs (derived from database UUIDs)\n• Allows tokens with different addresses on each chain to share the same token ID\n• Provides reverse lookup: token ID → address on specific chain\n• Tracks primary address for each token ID\n• Deployed on all testnets\n\n📊 GlobalSupplyTrackerV2:\n• Next-generation global supply tracker using Token IDs instead of addresses\n• Tracks global supply per token ID (not per address)\n• Chain-specific supply tracking per token ID\n• Automatic cross-chain synchronization via LayerZero\n• Backward compatible: Auto-looks up token IDs for address-based calls\n• Deployed on all testnets\n\n🎯 Why This Matters:\nTokens deployed on different chains have different contract addresses, but they represent the same logical token. The Token ID system creates a unified identity, allowing tokens with different addresses to share the same global supply and maintain perfect price synchronization.\n\n✅ Benefits:\n• True cross-chain price sync for tokens with different addresses\n• Unified token identity across all chains\n• Backward compatible with existing tokens\n• Future-proof architecture',
        },
        {
        ...
        {
          title: 'CrossChainSync - LayerZero Message Router (Legacy)',
          description: 'CrossChainSync is the original cross-chain communication hub (legacy, still supported):\n\n🌐 Responsibilities:\n• Receives supply updates from tokens on any chain\n• Validates updates using LayerZero security\n• Broadcasts updates to all other chains\n• Manages LayerZero endpoint IDs for each chain\n• Handles fee collection and distribution\n• Implements LayerZero v2 interfaces for maximum security\n\n💸 Fee Mechanism:\nWhen a user trades on a DEX, CrossChainToken collects a 0.5% fee. This fee is used to:\n• Pay LayerZero messaging costs\n• Fund cross-chain operations\n• Ensure sustainable synchronization\n\nThe user pays this fee automatically - no extra steps required!\n\n⚠️ Status: Legacy contract. New deployments use UnifiedCrossChainSync for dual protocol support.',
        },
        {
          title: 'How Contracts Work Together - The Cross-Chain Flow (Token ID System)',
          description: 'Here\'s exactly how the magic happens when someone buys your token (using the new Token ID system):\n\n1️⃣ User buys tokens on Ethereum via BondingCurve\n   → BondingCurve calculates price using GlobalSupplyTrackerV2\n   → V2 looks up token ID from TokenIDRegistry\n   → Tokens transferred to user\n   → GlobalSupplyTrackerV2 updates global supply using token ID\n\n2️⃣ GlobalSupplyTrackerV2 sends cross-chain message\n   → Looks up token address from TokenIDRegistry for current chain\n   → Calls CrossChainSync.syncSupplyUpdate() with token address\n   → Message contains: token address, new supply, source chain\n\n3️⃣ Cross-Chain Protocols Broadcast Message\n   → LayerZero v2 securely delivers to all other chains\n   → Supra HyperNova (when EVM support launches) provides redundancy\n   → Hedera HCS logs immutable audit record (Powered by Hedera)\n\n4️⃣ Other chains receive the update\n   → BSC, Base, Solana, Hedera all update their GlobalSupplyTrackerV2\n   → V2 looks up token ID from TokenIDRegistry using received address\n   → Updates global supply using the same token ID\n   → BondingCurves on all chains now show the new price\n   → Price is synchronized across ALL chains!\n\n⚡ Result: Perfect price consistency across Ethereum, BSC, Base, Solana, and Hedera in seconds!\n📝 Every sync event is immutably logged via Hedera Consensus Service for auditability.\n🆔 Token ID system ensures tokens with different addresses share the same global supply!',
        },
        {
          title: 'Cross-Chain Protocols & Hedera Integration - Secure Infrastructure',
          description: 'We use multiple cross-chain protocols for redundancy and Hedera for audit logging:\n\n🔒 LayerZero v2 Security:\n• Decentralized oracles and relayers\n• Messages are cryptographically verified\n• No single point of failure\n• Battle-tested with billions in value secured\n• Currently active and operational\n\n⚡ LayerZero Performance:\n• Messages delivered in seconds\n• Low gas costs (~$0.01-0.05 per message)\n• Reliable delivery guarantees\n• Supports all major chains\n\n🚀 Supra HyperNova (Coming Soon):\n• L1-to-L1 cryptographic consensus\n• No bridge trust assumptions\n• Enhanced security model\n• Sub-second finality (600-900ms)\n• EVM support expected Q1-Q2 2025\n• Will run in parallel with LayerZero for redundancy\n\n📝 Hedera Consensus Service:\n• Immutable, timestamped audit logs\n• Cryptographically verifiable ordering\n• Enterprise-grade auditability\n• Extremely low cost (~$0.0001 per message)\n• Carbon-negative infrastructure\n• No smart contract required for logging\n\n💾 Hedera File Service (HFS):\n• Decentralized storage for token metadata\n• Immutable files (cannot be deleted)\n• One-time cost (~$0.001 per file)\n• No monthly fees or pinning required\n• Permanent storage\n\n💰 Cost:\n• LayerZero fees: ~$0.01-0.05 per message\n• Supra fees: TBD (expected competitive)\n• Hedera HCS costs: ~$0.0001 per message\n• Hedera HFS: ~$0.001 one-time per file\n• Covered by the 0.5% DEX trade fee\n• Users don\'t pay extra - it\'s built into every trade\n• Sustainable model for long-term operation',
        },
        {
