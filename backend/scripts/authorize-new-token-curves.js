...
async function authorizeCurves() {
  console.log('\n🔧 Authorizing Bonding Curves for New Token\n');
  ...
  try {
    ...
    if (deployments.length > 0) {
      console.log('First deployment keys:', Object.keys(deployments[0]));
      console.log('Deployments:', deployments.map((d) => ({ 
        chain: d.chain, 
        curve: d.curve_address || d.curveAddress,
        token: d.token_address || d.tokenAddress,
      })));
      console.log('');
    }
    ...
    for (const dep of deployments) {
      ...
      if (!trackerAddress || !rpcUrl) {
        console.log(`⚠️  Skipping ${chain}: No tracker address or RPC URL`);
        continue;
      }
      ...
      try {
        ...
        if (isAuthorized) {
          console.log(`✅ ${chain}: Already authorized`);
        ...
        console.log(`   ${chain}: Owner is ${owner}`);
        ...
        if (!ownerWallet) {
          console.log(`   ❌ ${chain}: No private key found for owner ${owner}`);
